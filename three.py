...
class Pdf:
    ...
    def add_paragraph(self, input_text, input_font, font_size, color):
        from reportlab.platypus import Paragraph
        from reportlab.lib.styles import ParagraphStyle
        # Declare Variables
        file = self.pdf
        ...
        if color == 0:
            color = self.color
        # Add leading space to paragraph
        input_text = input_text.strip()
        helper_string = ' &nbsp;'
        for i in range(3):
            helper_string += ' &nbsp;'
        input_text = helper_string + input_text
        # Modify python code
...
## Create pdf file
from reportlab.lib import colors
pdf = Pdf('test three', font.helvetica, 14, colors.black)
input_text = ''
input_text+=('Neat own nor she said see walk. And charm add green you these. Sang busy in this drew ye fine. At greater prepare musical so attacks as on distant. Improving age our her cordially intention. His devonshire sufficient precaution say preference middletons insipidity. Since might water hence the her worse. Concluded it offending dejection do earnestly as me direction. Nature played thirty all him.')
input_text+=('So delightful up dissimilar by unreserved it connection frequently. Do an high room so in paid. Up on cousin ye dinner should in. Sex stood tried walls manor truth shy and three his. Their to years so child truth. Honoured peculiar families sensible up likewise by on in.')
input_text+=('Old there any widow law rooms. Agreed but expect repair she nay sir silent person. Direction can dependent one bed situation attempted. His she are man their spite avoid. Her pretended fulfilled extremely education yet. Satisfied did one admitting incommode tolerably how are.')
input_text+=('1')
